...
complementary~\cite{osc2026layered}. Our evaluation shows 24 verified \fstar{}
specifications, 177 Python runtime tests, 10 incorrectness lemmas, and
...
retain only evidence-bearing roles (\texttt{RoleTool} $\to$
\texttt{ToolOutput}, \texttt{RoleUser} $\to$ \texttt{ExternalInput});
system and assistant messages produce no observations.
...
\midrule
\fstar{} specifications    & 24 & All verified (zero admits) \\
Python runtime tests        & 177 & All pass \\
OWASP ASI06 vectors         & 23 + 8 benign & Full coverage \\
Normalization tests         & 20 assert\_norm & Type-checker evaluated \\
Incorrectness lemmas (core) & 7 (L1--L7) & All verified \\
Incorrectness lemmas (proxy) & 3 (L\textsubscript{P1}--L\textsubscript{P3}) & All verified \\
...
\texttt{PraxisLemmas.fst} contains 7 core lemmas (L1--L7) that prove
rejection is \emph{necessary} under specific attack conditions, and
\texttt{ProxyLemmas.fst} adds 3 proxy lemmas (L\textsubscript{P1}--L\textsubscript{P3})
...
evidence. The proxy lemmas complement this: L\textsubscript{P2} proves that
the automatic classifier never misidentifies hallucinations as verbatim
relay, and L\textsubscript{P3} proves that evidence-free conclusions are
always flagged as derivations regardless of their content.
...
(7 core + 3 proxy) prove that rejection is necessary under attack
