...
framework on Red Hat OpenShift AI with vLLM/KServe model serving and NVIDIA
OpenShell sandbox enforcement. Our evaluation shows 24 verified \fstar{}
...
\begin{lstlisting}[language={},caption={Deployment architecture on OpenShift AI.},label={lst:arch}]
+- OpenShift AI ----------------------------+
|                                            |
| vLLM/KServe InferenceService (GPU)        |
|       ^                                    |
|       | HTTPS (OSSM mTLS, ODH CA)          |
| +-----+----------------------------------+|
...
| |                                       | |
| | +- Hermes Agent (UBI 9) -----------+  | |
| | | MEMORY.md, USER.md, SQLite       |  | |
| | +-------------+--------------------+  | |
| |               | gRPC MemoryIntent     | |
| | +-------------v--------------------+  | |
| | | Praxis Sidecar (:50052)          |  | |
| | | F*/Z3 -> cert or rejection       |  | |
| | | OTel span -> MLflow audit        |  | |
| | +----------------------------------+  | |
| | Policy Engine (seccomp, eBPF)         | |
| +---------------------------------------+ |
|                                            |
...
vLLM/KServe, and OpenShell, extending RHOAI TrustyAI from model-level trust
to agent-level memory groundedness.
